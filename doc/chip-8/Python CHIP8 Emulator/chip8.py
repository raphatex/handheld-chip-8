...
class SoundTimer(DelayTimer):
    ...
    def beep(self):
        if self.timer > 1:
            winsound.Beep(440, int(self.timer / 60))
            self.timer = 0
...
chip8 = Emulator()
chip8.readProg("G:\\Mon Drive\\projet chip 8\\emulateur\\chip8-test-suite-main\\src\\tests\\6-keypad.8o")
chip8.mainLoop()
