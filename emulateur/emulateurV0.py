...
from utime import sleep_us, ticks_cpu, ticks_us, ticks_diff, ticks_ms
...
def getKeys():
    return [p0.value(), p1.value(), p2.value(), p3.value(), p4.value(), p5.value()]
            ...
            for j in range(8):
                try:
                    ...
                    if int(spriteBits[i][j]) == 1:
                        display.fill_rectangle((Vx + j) * self.size, (Vy + i) * self.size, self.size, self.size, invert=inversion1)
...
display = Display(spi, dc=Pin(29), cs=Pin(28), rst=Pin(27))
